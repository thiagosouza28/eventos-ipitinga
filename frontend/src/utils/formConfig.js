...
export const isSystemField = (id) => SYSTEM_FIELD_IDS.has(id);
...
export const normalizeFormConfig = (input) => {
    ...
    const filtered = normalized.campos.filter((field) => {
        if (!field.id || !field.label || !FORM_FIELD_TYPES.includes(field.tipo))
            return false;
