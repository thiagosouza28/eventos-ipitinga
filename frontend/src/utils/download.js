 ...
 */
export const isValidBlob = (blob) => {
    return blob instanceof Blob && blob.size > 0;
};
/**
 * Detecta tipo de erro comum em downloads
 */
export const classifyDownloadError = (error) => {
    if (error.status === 401)
        return 'auth';
    if (error.status === 403)
        return 'permission';
 ...
 */
export const sanitizeFileName = (fileName, fallback = 'download') => {
 ...
 */
export const extractFileName = (contentDisposition, fallback = 'download') => {
 ...
 */
export const detectFileType = (contentType, fileName) => {
    if (contentType) {
