import { ref } from 'vue';
import { classifyDownloadError, detectFileType, extractFileName, getErrorMessage, isValidBlob, sanitizeFileName, triggerFileDownload } from '../utils/download';
/**
...
export const useFileDownload = () => {
     ...
     */
    const downloadFile = (blob, fileName, options) => {
        try {
            if (!isValidBlob(blob)) {
     ...
     */
    const downloadFromResponse = (response, defaultFileName, options) => {
        try {
     ...
     */
    const handleDownloadError = (error, context = 'download') => {
