...
type SystemConfigState = {
  ...
  initialized: boolean;
  error: string | null;
  setConfig: (payload?: unknown) => void;
  initialize: (force?: boolean) => Promise<void>;
  refresh: () => Promise<void>;
  saveConfig: (payload: PartialSystemConfigSettings) => Promise<void>;
};
...
export const useSystemConfigStore = create<SystemConfigState>((set, get) => ({
  ...
  error: null,
  setConfig: (payload) => {
    const resolved = resolveSettingsPayload(payload ?? defaultSystemConfig);
    set({ config: resolved });
    applyCssVariables(resolved);
  },
  initialize: async (force = false) => {
    if (get().initialized && !force) {
      applyCssVariables(get().config);
      return;
    }
    set({ loading: true });
    try {
      ...
      set({ error: "Não foi possível carregar configurações. Usando padrão." });
      get().setConfig(defaultSystemConfig);
    } finally {
